...
class ATDNet(nn.Module):
    def __init__(self, channels, num_of_layers=15):
        super(ATDNet, self).__init__()
        kernel_size = 3
        padding = 1
        features = 64
        groups = 1
        ...
        self.conv1_15 = nn.Sequential(
            nn.Conv2d(in_channels=features, out_channels=features, kernel_size=kernel_size, padding=1, groups=groups,
                      bias=False), nn.BatchNorm2d(features), nn.ReLU(inplace=True))
        self.conv1_16 = nn.Conv2d(in_channels=features, out_channels=1, kernel_size=3, padding=1, groups=groups,
                                  bias=False)
        ...
        self.conv2_15 = nn.Sequential(
            nn.Conv2d(in_channels=features, out_channels=features, kernel_size=kernel_size, padding=1, groups=groups,
                      bias=False), nn.BatchNorm2d(features), nn.ReLU(inplace=True))
        self.conv2_16 = nn.Sequential(
            nn.Conv2d(in_channels=features, out_channels=features, kernel_size=kernel_size, padding=1, groups=groups,
                      bias=False), nn.BatchNorm2d(features), nn.ReLU(inplace=True))
        self.conv2_17 = nn.Conv2d(in_channels=features, out_channels=1, kernel_size=3, padding=1, groups=groups,
                                  bias=False)


        self.conv3_2 = nn.Conv2d(in_channels=3, out_channels=1, kernel_size=1, padding=0, groups=groups, bias=True)
